...
// Tokener is an interface for obtaining anonymous tokens
...
// GetToken returns an anonymous token from the auth0 client
func (a Auth0Tokener) GetToken(ctx context.Context) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed obtaining new anonymous access token: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return "", fmt.Errorf("failed obtaining new anonymous access token: status code %v", resp.StatusCode)
	}

	var tr tokenResponse
	err = json.NewDecoder(resp.Body).Decode(&tr)
	if err != nil {
		return "", fmt.Errorf("failed decoding new anonymous access token: %w", err)
	}
